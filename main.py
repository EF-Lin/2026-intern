...
def main():
    ...
    for i in tqdm(times):
        ...
        name = f"{stations[0]}_to_{stations[1]}_waterfall_plot_{i.replace(':', '')}-gif-c"#["Time"]
        ...
        fa = Fall([wa.pa], filename=name, title=[f"Hole A {str(wa.pa.attrs.time_min.astype("datetime64[m]")).replace('T', ' ')} Waterfall Plot"], figsize=(9, 3))
        fa.gif(d=10, dpi=150)
